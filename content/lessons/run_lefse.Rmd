...
atacama.ps.rds %>%
  ...
  path(outdir, .) ->
  lefse.input.file
lefse.bar=path_ext_set(lefse.input.file, ".png")
...
GenerateLefseOutput = function(ps,output_columns,outfile){
  stopifnot(length(output_columns)>=1,
            length(output_columns)<=2)
...
format.flags = GenerateLefseOutput(atacama.ps, "TransectName", lefse.input.file)
